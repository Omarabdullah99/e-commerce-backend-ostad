...
const UserOTP=async(req,res)=>{
    try {
      ...
      let result=  await UserModel.updateOne({email:email},{$set:{otp:code}}, {upsert:true})
      res.status(200).json({status:"success",message:"6 digit OTP has been send", result:result})
    } catch (error) {
        res.status(400).json(error)
    }
}
...
const LoginOtpVerify = async (req, res) => {
    try {
        ...
        if (total === 1) {
            ...
            return res.status(200).json({ status: "success", message: "Valid OTP", token: token });
...
module.exports={CreateUser,GetAllUsers,CreateProfileUser,GetAllProfileUser,CreateReview,GetAllReviews,UserOTP,LoginOtpVerify}